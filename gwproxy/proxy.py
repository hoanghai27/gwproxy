...
class TcpGwProxy:
    ...
    def start(self, callback_cls):
        ...
        while True:
            ...
            self._on_accept(client, address, callback_cls)

    def _on_accept(self, sock, address, callback_cls):
        sock.settimeout(60)
        ip, port = get_dst_address(sock)
        print('- New connection: %s:%s -> %s:%s' % (address[0], address[1], ip, port))
        proxy_thread = TcpProxyThread(sock, ip, port, callback_cls, self.auto_fw)
        proxy_thread.start()
